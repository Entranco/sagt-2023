\begin{frame}[t]
	...
	\input{figures/table_legend.tex}
...
\begin{frame}[t]
	...
	\begin{itemize}
		...
		\only<3>{
			$\forall S_i \in \mathcal{F}$, add $s_i$ with a unique color ($+m$ candidates)
		}

		\only<4-6>{
			$\forall e \in \mathcal{U}$, add $d$ vertices with a unique color ($+n$ candidates)
		}
